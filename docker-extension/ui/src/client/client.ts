...
export class DockerClient {
    ...
    static async getContainers(): Promise<Array<Container>> {
        const containers: any = await ddClient.docker.listContainers();
        ...
        for (var i = 0; i < containers.length; i++) {
            ...
            const [hasRepositoryAddress, goshRepositoryAddress] =
                DockerClient.readContainerImageMetadata(
                    container,
    ...
    static async getImages(): Promise<Array<Image>> {
        const images: any = await ddClient.docker.listImages();
    ...
    static async getImageStatus(
        ...
        try {
            if (!ddClient.extension.vm) throw new Error('Extension vm undefined');
