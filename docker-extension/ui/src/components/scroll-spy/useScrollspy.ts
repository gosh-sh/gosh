...
export const useScrollspy = ({
  ...
  const isScrolledPastViewport = useCallback(
    (element: Element) => {
      if (!rootEl.current) {
        return false;
      }
      ...
      const elementScrollTop =
        rootEl.current instanceof RootEl
          ? innerScrollTop + elementRect.top - rootEl.current.top + offset
          : innerScrollTop + elementRect.top + offset;
      return elementScrollTop < innerScrollTop;
    },
    [rootEl, offset]
  );
  ...
  const getElementsScrolled = useCallback(() => {
    return sectionRefs.map(sectionRef => {
      if (sectionRef.current) {
        return isScrolledPastViewport(sectionRef.current);
      }
      return false;
    });
  }, [isElementInViewport, sectionRefs]);
  ...
  const [elementsScrolled, updateElementsScrolled] = useState<
    boolean[]
  >([]);
  ...
  const spy = useCallback(() => {
    ...
    const newElementsScrolled = getElementsScrolled();
    updateElementsScrolled(newElementsScrolled);
  ...
  return useMemo(
    ...
    [elementsStatusInViewport, elementsScrolled, currentElementIndexInViewport],
  );
};
