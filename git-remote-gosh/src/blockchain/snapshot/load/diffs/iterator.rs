                ...
                while index.is_none() { 
                    ...
                    if index.is_none() { 
                        ...
                        match next_page_info {
                            Some(next_page_info) => {
                                cursor = Some(next_page_info);
                            },
                            _ => panic!("We reached the end of the messages queue to a snapshot and were not able to find original commit there.")
                        }
                    } else {
                        log::info!("Commit found at {}", index.unwrap());
                        _next_page_info = next_page_info;
                    }
                }
                self.buffer_cursor = index.unwrap();
                match _next_page_info {
