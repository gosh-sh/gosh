...
class AppRotateHandler extends AppHandler_1.default {
    ...
    async handle(debug) {
        ...
        let trash_count = 0;
        const call = this.redlock ?
            (f) => this.redlock.using([lck], 5000, {
                retryCount: Math.round(this.lock_retry_s / (this.redlock_retry_delay_ms / 1000))
            }, f) :
            (f) => f();
        return await call(async () => await this.doSteps(
        ...
        }, 'wait for spinner gone', /*14*/ () => this.waitForGone('svg.fa-spin', this.longtimeout_ms), 'click code branch icon', /*15*/ () => this.click("div.items-center > svg.fa-code-branch"), 'input src branch name', /*16*/ () => this.type("//input[@type='text' and @placeholder='Search branch']", this.origin), 'click branch item', /*17*/ () => this.click(`//li[contains(., '${this.origin}') and @role='option']`), 'wait 500ms', () => this.wait(500), 'input dst branch name', /*18*/ () => this.type("//input[@name='newName' and @placeholder='Branch name']", this.branch), 'click create br button', /*19*/ () => this.click("//button[contains(., 'Create branch') and @type='submit']"), 
        ...
        /*27*/ () => this.waitFor("div.bg-gray-100 > div > button > svg.fa-copy", this.longtimeout_ms), 'wait 1000ms', () => this.wait(1000), 'close page', /*28*/ () => this.closePage(), 'wait 10000ms', () => this.wait(10000), 
        /*29 - 40*/ ...this.initialSteps(debug), 'click copy icon', /*41*/ () => this.click("div.bg-gray-100 > div > button > svg.fa-copy"), 'check file contents', /*42*/ async () => {
