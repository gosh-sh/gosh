...
class AppSetupHandler extends AppHandler_1.default {
    ...
    async handle(debug) {
        ...
        /* 0 -  7 */ ...this.initialSteps(debug, AppHandler_1.default.userSteps), 'wait for spinner gone', /* 8*/ () => this.waitForGone('svg.fa-spin'), 'count wanted organizations', /* 9*/ async () => { found.org = (await this.count(`//a[@href='/${this.organization}']`)) == 1; }, 'organization exists', ...this.cond_ifnot(() => found.org, [
        ...
        ]), 'click organization link', /*13*/ () => this.click(`//a[@href='/${this.organization}']`, 180000), 'wait for spinner gone', /*14*/ () => this.waitForGone('svg.fa-spin'), 'count repositories', /*15*/ async () => { found.rep = (await this.count(`//a[@href='${ort}']`)) == 1; }, 'repository exists', ...this.conditional(() => found.rep, [
        ...
        ]), 'click branches link', /*20*/ () => this.click(`//a[@href='${ort}/branches']`, 180000), 'wait for main br link', /*21*/ () => this.waitFor(`//a[@href='${ort}/tree/main']`), 'count wanted branches', /*22*/ async () => { found.root = (await this.count(`//a[@href='${ort}/tree/${this.branch}']`)) == 1; }, 'branch exists', ...this.cond_ifnot(() => found.root, [
        ...
        ]), 'click branch link', /*26*/ () => this.click(`//a[@href='${ort}/tree/${this.branch}']`), 'wait for spinner gone', /*27*/ () => this.waitForGone('svg.fa-spin'), 'count wanted files', /*28*/ async () => { found.file = (await this.count(`//a[@href='${ort}/blobs/${bfn}' or @href='${ort}/blobs/view/${bfn}']`)) == 1; }, 'file exists', ...this.cond_ifnot(() => found.file, [
        ...
        ]), 'click branches link', /*34*/ () => this.click(`//a[@href='${ort}/branches']`, 180000), 'wait for main br link', /*35*/ () => this.waitFor(`//a[@href='${ort}/tree/main']`), ...this.for_each(this.branches, 'branches', (s) => [
