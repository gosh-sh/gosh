...
class AppSetupHandler extends AppHandler_1.default {
    ...
    async handle(debug) {
        ...
        return await this.doSteps(...this.initialSteps(debug, AppHandler_1.default.indexSteps), 'remove footer', () => this.removeFooter(), // not a step, utility function
        'click signin', () => this.click(`//a[${(0, Utils_1.ac_hrefs)('/a/signin')}]`), 'input seed', () => this.pasteInto("//textarea[@name='phrase']", this.seed, undefined, undefined, true), 'input username', () => this.pasteInto("//input[@name='username']", this.username, 50, true), 'click sign in', () => this.click("//button[contains(., 'Sign in') and @type='submit']"), 'wait for spinner gone', () => this.waitForGone('svg.fa-spinner'), 'wait 100ms', () => this.wait(500), 'count popup toast errors', async () => { found.err = (await this.count(`//div[@role='alert' and @class='Toastify__toast-body']`)) == 1; }, 'fail if error found', async () => { if (found.err)
            throw new Error('Please deploy and configure user profile first!'); }, 'input pin code', () => this.type("//input[@type='password' and @placeholder='PIN code']", "1111"), 'wait 200ms', () => this.wait(200), 'confirm pin code', () => this.type("//input[@type='password' and @placeholder='PIN code']", "1111"), 'wait for spinner gone', () => this.waitForGone('svg.fa-spinner'), 'wait 100ms to settle', () => this.wait(100), 'wait for spinner gone', () => this.waitForGone('svg.fa-spinner'), 'search for organization', () => this.type('//input[@type="search"]', or), 'count wanted organizations', async () => { found.org = (await this.count(`//a[${(0, Utils_1.or_hrefs)(`/o/${or}`)}]`)) == 1; }, 'organization exists', ...this.cond_ifnot(() => found.org, [
