...
export default class AppRotateHandler extends AppHandler {
    ...
    protected lock_retry_s: number = 60;
    ...
    async handle(debug: boolean): Promise<MetricsMap> {
        ...
        async() =>
            await this.doSteps(
                ...
                'input dst branch name',   /*18*/ () => this.type("//input[@name='newName' and @placeholder='Branch name']", this.branch),
                ...
                'click dst branch',        /*21*/ () => this.click(`//a[${or_hrefs(`/o/${or}/r/${re}/tree/${br}`)}]`, this.longtimeout_ms),
                'click file',              /*22*/ () => this.click(`//a[${or_hrefs(`/o/${or}/r/${re}/blobs/view/${br}/${fn}`)}]`),
                ...
                'input commit title',      /*25*/ () => this.type("//input[@name='title' and @placeholder='Commit title']", `Update ${fn} (${niso()})`),
