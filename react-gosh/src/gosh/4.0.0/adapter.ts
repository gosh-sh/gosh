...
class GoshAdapter_4_0_0 implements IGoshAdapter {
  private static instance: GoshAdapter_4_0_0
  ...
  static getInstance(
    ...
    if (!GoshAdapter_4_0_0.instance) {
      GoshAdapter_4_0_0.instance = new GoshAdapter_4_0_0(goshroot, goshaddr)
    }
    return GoshAdapter_4_0_0.instance
  }
  ...
  getVersion(): string {
    return GoshAdapter_4_0_0.version
  }
...
class GoshDaoAdapter implements IGoshDaoAdapter {
  ...
  private _getMembers(mapping: {
    [key: string]: { member: TAddress; count: string }
  }) {
    const members = []
    ...
    const cells = await executeByChunk(
      ...
      async ({ type, params }) => {
        ...
        if (type === ESmvEventType.DAO_EVENT_ALLOW_DISCUSSION) {
          return await this.updateEventAllowDiscussion({
            ...params,
            cell: true,
          })
        }
        ...
        if (type === ESmvEventType.DAO_ASK_MEMBERSHIP_ALLOWANCE) {
          return await this.updateAskMembershipAllowance({
            ...params,
            cell: true,
          })
        }
...
class GoshRepositoryAdapter implements IGoshRepositoryAdapter {
  ...
  async getCommit(options: {
    ...
    const { branch, sha, parents, content, initupgrade, isCorrectCommit } =
      details
  ...
  async getCommitBlob(
    ...
    const current = await this._getCommitBlob(
      commit,
      treepath,
      content,
      approved,
  ...
  async getCommitBlobs(
    ...
    return await executeByChunk<
      TAddress,
      { address: string; treepath: string }
    >(Array.from(new Set(addresses)), MAX_PARALLEL_READ, async (address) => {
      const snapshot = await this._getSnapshot({ address })
  ...
  async deleteBranch(
    ...
    const snapCodeHash = await this.client.boc.get_boc_hash({
      boc: snapCode.value0,
    })
    const accounts = await getAllAccounts({
