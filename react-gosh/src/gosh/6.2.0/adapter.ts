...
class GoshAdapter_6_2_0 implements IGoshAdapter {
  private static instance: GoshAdapter_6_2_0
  private auth?: { username: string; keys: KeyPair }

  static version: string = '6.2.0'
  ...
  static getInstance(
    ...
    if (!GoshAdapter_6_2_0.instance) {
      GoshAdapter_6_2_0.instance = new GoshAdapter_6_2_0(goshroot, goshaddr)
    }
    return GoshAdapter_6_2_0.instance
  }
  ...
  async isValidUser(
    user: TUserParam,
  ): Promise<{ user: TUserParam; address: string }> {
    if (user.type === 'dao') {
  ...
  getVersion(): string {
    return GoshAdapter_6_2_0.version
  }
  ...
  private _isValidName(name: string, field?: string): TValidationResult {
    ...
    if (matchHyphens && matchHyphens.length > 0) {
      return {
        valid: false,
        reason: `${field} can not contain consecutive "-"`,
      }
    }
    ...
    if (matchUnderscores && matchUnderscores.length > 0) {
      return {
        valid: false,
        reason: `${field} can not contain consecutive "_"`,
      }
    }
...
class GoshDaoAdapter implements IGoshDaoAdapter {
  ...
  async isMember(params: TIsMemberParams): TIsMemberResult {
    ...
    if (user) {
      ...
      throw new GoshError(
        'Either profile address or username should be provided',
      )
    }
  ...
  async getNext() {
    ...
    const curVersion = this.getVersion()
    const nextVersions = Object.keys(AppConfig.versions).filter(
      (k) => k > curVersion,
    )
    for (const version of nextVersions) {
      const sc = GoshAdapterFactory.create(version)
  ...
  async getDetails(): Promise<TDao> {
    ...
    const { _isTaskRedeployed } = await this.dao.runLocal(
      '_isTaskRedeployed',
      {},
    )
    ...
    return {
      ...
      isAuthOwner:
        this.profile && this.profile.address === owner ? true : false,
  ...
  async getRemoteConfig(): Promise<object> {
    if (
      !this.profile ||
      !this.wallet ||
      this.wallet.account.signer.type !== 'Keys'
    ) {
  ...
  async getTask(options: {
    ...
    if (details.candidates.length) {
      ...
      team = {
        commit: commit
          ? { branch: commit.branch, name: commit.name }
          : undefined,
        assigners,
        reviewers,
        managers,
  ...
  async getPrevDao() {
    const { value0: prevAddr } = await this.dao.runLocal(
      'getPreviousDaoAddr',
      {},
    )
    if (!prevAddr) {
    ...
    const prevDao = await this.gosh.getDao({
      address: prevAddr,
      useAuth: false,
    })
  ...
  ): Promise<TDaoMemberCreateResult> {
  ...
  ): Promise<TDaoMemberDeleteResult> {
  ...
  async sendInternal2Internal(user: TUserParam, amount: number): Promise<void> {
    ...
    await this.wallet.run('sendToken', {
      pubaddr: profile.address,
      grant: amount,
    })
  }
  ...
  ): Promise<TDaoTokenDaoLockResult> {
    const {
      wallet,
      isLock,
      amount,
  ...
  async approveBigTask(
    params: TBigTaskApproveParams,
  ): Promise<TBigTaskApproveResult> {
  ...
  async upgradeBigTask(
    params: TBigTaskUpgradeParams,
  ): Promise<TBigTaskUpgradeResult> {
    const {
      repositoryName,
      name,
      prevVersion,
  ...
  async deleteSubTask(
    params: TSubTaskDeleteParams,
  ): Promise<TSubTaskDeleteResult> {
  ...
  private async _getSystemRepository(
    ...
    if (
      ...
      this.systemRepository.getVersion() !== version
    ) {
...
class GoshRepositoryAdapter implements IGoshRepositoryAdapter {
  ...
  async getBlob(options: {
    ...
    const ipfscid =
      temporaryCommit === commitName ? temporaryIpfs : approvedIpfs
    result.onchain = {
  ...
  async getCommitBlob(
    ...
    if (!(await snapshot.isDeployed())) {
      ...
      data.current = await this._getCommitBlob(
        commit,
        treepath,
        content,
        approved,
  ...
  async getCommitBlobs(
    ...
    const { messages } = await object.getMessages(
      { msgType: ['IntIn'] },
      true,
      true,
    )
  ...
  async getBranch(name: string): Promise<TBranch> {
    const { branchname, commitaddr, commitversion } =
      await this._getBranch(name)
  ...
  async subscribeIncomingCommits(
    ...
    await this.repo.account.subscribeMessages(
      'body msg_type',
      async (message) => {
  ...
  async push(
    ...
    const { commitHash, commitContent, commitParents } =
      await this._generateCommit(
        branchTo,
        updatedTree0.sha1,
        message,
        branchParent,
    ...
    await this._runMultiwallet(
      blobsData,
      async (wallet, { status, data }, index) => {
        if (
          data.patch ||
          (status === 1 && data.isGoingIpfs) ||
          data.isGoingOnchain
        ) {
  ...
  async createCommitTag(
    params: TRepositoryCreateCommitTagParams,
  ): Promise<void> {
  ...
  private async _getTreeSha256(params: { mapping?: any; items?: TTreeItem[] }) {
    ...
    const mapping =
      params.mapping || (await this._getTreeItemsMapping(params.items!))
    const { value0 } = await this.auth.wallet0.runLocal(
  ...
  private async _getTreeBlobs(
    ...
    { address: string; treepath: string; content: string | Buffer }[]
  > {
  ...
  private async _deployCommit(
    ...
    await this.auth.wallet0.run('deployCommit', {
      ...
      commitName: commit,
      fullCommit: content,
      parents: parents.map(({ address, version }) => ({
        addr: address,
        version,
      })),
  ...
  private _getTreeItemsFromPath(
    ...
    while (path !== '') {
      ...
      if (
        !items.find((item) => item.path === dirPath && item.name === dirName)
      ) {
        items.push({
          flags: 0,
          mode: '040000',
...
class GoshSmvAdapter implements IGoshSmvAdapter {
  ...
  async getEventTime(params: {
    ...
    const { realFinishTime } = await event.runLocal(
      'realFinishTime',
      {},
      undefined,
      {
  ...
  async getWalletBalance(wallet: IGoshWallet): Promise<number> {
    const { m_pseudoDAOBalance } = await wallet.runLocal(
      'm_pseudoDAOBalance',
      {},
    )
  ...
  private async _getEventData(
    ...
    if (type === ESmvEventType.DAO_UPGRADE) {
      ...
      const { num, data0 } = await event.runLocal(
        'getDataFirst',
        {},
        undefined,
        {
    ...
    const decoded = await event.runLocal(fn, {}, undefined, {
      useCachedBoc: true,
    })
