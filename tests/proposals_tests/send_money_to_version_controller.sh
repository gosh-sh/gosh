...
VC_ABI="../v4_x/contracts/gosh/versioncontroller.abi.json"
VC_ADDR=$(cat ../v4_x/contracts/gosh/VersionController.addr)
VC_SEED=$(cat ../v4_x/contracts/gosh/gosh.seed | grep Seed | cut -d ':' -f 2)
...
cmd="tonos-cli callx --abi $VC_ABI --addr $VC_ADDR --keys $VC_SEED -m getMoneyFromSystemContract '{\"version\":\"4.0.0\",\"value\":1000000000000}'"
echo $cmd
eval $cmd
...
cmd="tonos-cli callx --abi $VC_ABI --addr $VC_ADDR --keys $WALLET_KEYS -m getMoneyFromSystemContract '{\"version\":\"4.0.0\",\"value\":1000000000000}'"
echo $cmd
if eval $cmd; then
