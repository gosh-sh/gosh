...
TREE_ADDR=$(tonos-cli -j run "$REPO_ADDR" getTreeAddr "{\"treeName\":\"1.txt\"}" --abi "$REPO_ABI" | sed -n '/value0/ p' | cut -d'"' -f 4)
...
tonos-cli callx --addr "$WALLET_ADDR" --abi "$WALLET_ABI" --keys "$WALLET_KEYS" -m deployNewSnapshot --branch "main" --commit "" --repo "$REPO_ADDR"  --snapshotipfs null --name "1.txt" --snapshotdata ""

SNAPSHOT_ADDR=$(tonos-cli -j run "$WALLET_ADDR" getSnapshotAddr "{\"branch\":\"main\",\"repo\":\"$REPO_ADDR\",\"name\":\"1.txt\"}" --abi "$WALLET_ABI" | sed -n '/value0/ p' | cut -d'"' -f 4)
...
COMMIT_ADDR=$(gosh-cli runx --abi $REPO_ABI --addr $REPO_ADDR -m getAllAddress | jq '.value0[0].commitaddr' | cut -d'"' -f 2)

tonos-cli callx --addr "$WALLET_ADDR" --abi "$WALLET_ABI" --keys "$WALLET_KEYS" -m deployCommit "{\"repoName\":\"$REPO_NAME\",\"branchName\":\"main\",\"commitName\":\"$COMMIT_ID\",\"fullCommit\":\"tree 3cc611957f6b92c2c77e6d4704d3bcf85a6da915\nauthor My name <foo@bar.com> 1679605778 +0300\ncommitter My name <foo@bar.com> 1679605778 +0300\n\ntest\n\",\"parents\":{\"addr\":\"$COMMIT_ADDR\",\"version\":\"2.0.0\"},\"tree\":\"$TREE_ADDR\",\"upgrade\":false}"
