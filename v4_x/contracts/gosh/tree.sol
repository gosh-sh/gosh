...
contract Tree is Modifiers {
    ...
    function addTree(address pubaddr, uint128 index, mapping(uint256 => TreeObject) tree1, bool isFinal) public {
        require(checkAccess(pubaddr, msg.sender, index), ERR_SENDER_NO_ALLOWED);
        tvm.accept();
        require(_isReady == false, ERR_PROCCESS_END);
        getMoney();
        this.addTreeself{value: 0.2 ton, flag: 1}(uint256(0), tree1, isFinal);
    }

    function addTreeself(uint256 index, mapping(uint256 => TreeObject) tree1, bool isFinal) public senderIs(address(this)){
        tvm.accept();
        require(_isReady == false, ERR_PROCCESS_END);
        ...
        if (res.hasValue()) {
            ...
            (index, obj) = res.get();
            _tree[index] = obj;
            this.addTreeself{value: 0.2 ton, flag: 1}(index, tree1, isFinal);
        }
        else {
            _isReady = isFinal;
        }
    ...
    //Getters
    function getDetails() external view returns(bool, mapping(uint256 => TreeObject), optional(string), uint256, string, address){
        return (_isReady, _tree, _ipfs, _shaTreeLocal, _shaTree, _pubaddr);
    }
