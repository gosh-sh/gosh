...
/opt/gosh/contracts/tonos-cli genphrase > $GOSH_PATH/$VERSIONCONTROLLER_SEED_FILE_OUT
...
# Get codes
# ############################################################
SYSTEMCONTRACT_CODE=$(/opt/gosh/contracts/tonos-cli -j decode stateinit --tvc $GOSH_PATH/systemcontract.tvc | tr -d ' ",' | sed -n '/code:/s/code://p')
TOKEN_ROOT_CODE=$(/opt/gosh/contracts/tonos-cli -j decode stateinit --tvc $SMV_PATH/TokenRoot.tvc | tr -d ' ",' | sed -n '/code:/s/code://p')
TOKEN_WALLET_CODE=$(/opt/gosh/contracts/tonos-cli -j decode stateinit --tvc $SMV_PATH/TokenWallet.tvc | tr -d ' ",' | sed -n '/code:/s/code://p')
TOKEN_LOCKER_CODE=$(/opt/gosh/contracts/tonos-cli -j decode stateinit --tvc $SMV_PATH/SMVTokenLocker.tvc | tr -d ' ",' | sed -n '/code:/s/code://p')
LOCKER_PLATFORM_CODE=$(/opt/gosh/contracts/tonos-cli -j decode stateinit --tvc $SMV_PATH/LockerPlatform.tvc | tr -d ' ",' | sed -n '/code:/s/code://p')
SMV_CLIENT_CODE=$(/opt/gosh/contracts/tonos-cli -j decode stateinit --tvc $SMV_PATH/SMVClient.tvc | tr -d ' ",' | sed -n '/code:/s/code://p')
SMV_PROPOSAL_CODE=$(/opt/gosh/contracts/tonos-cli -j decode stateinit --tvc $SMV_PATH/SMVProposal.tvc | tr -d ' ",' | sed -n '/code:/s/code://p')
DIFF_CODE=$(/opt/gosh/contracts/tonos-cli -j decode stateinit --tvc $GOSH_PATH/diff.tvc | tr -d ' ",' | sed -n '/code:/s/code://p')
REPO_CODE=$(/opt/gosh/contracts/tonos-cli -j decode stateinit --tvc $GOSH_PATH/repository.tvc | tr -d ' ",' | sed -n '/code:/s/code://p')
COMMIT_CODE=$(/opt/gosh/contracts/tonos-cli -j decode stateinit --tvc $GOSH_PATH/commit.tvc | tr -d ' ",' | sed -n '/code:/s/code://p')
SNAPSHOT_CODE=$(/opt/gosh/contracts/tonos-cli -j decode stateinit --tvc $GOSH_PATH/snapshot.tvc | tr -d ' ",' | sed -n '/code:/s/code://p')
WALLET_CODE=$(/opt/gosh/contracts/tonos-cli -j decode stateinit --tvc $GOSH_PATH/goshwallet.tvc | tr -d ' ",' | sed -n '/code:/s/code://p')
DAO_CODE=$(/opt/gosh/contracts/tonos-cli -j decode stateinit --tvc $GOSH_PATH/goshdao.tvc | tr -d ' ",' | sed -n '/code:/s/code://p')
TREE_CODE=$(/opt/gosh/contracts/tonos-cli -j decode stateinit --tvc $GOSH_PATH/tree.tvc | tr -d ' ",' | sed -n '/code:/s/code://p')
TAG_CODE=$(/opt/gosh/contracts/tonos-cli -j decode stateinit --tvc $GOSH_PATH/tag.tvc | tr -d ' ",' | sed -n '/code:/s/code://p')
CONTENTSIG_CODE=$(/opt/gosh/contracts/tonos-cli -j decode stateinit --tvc $GOSH_PATH/content-signature.tvc | tr -d ' ",' | sed -n '/code:/s/code://p')
TASK_CODE=$(/opt/gosh/contracts/tonos-cli -j decode stateinit --tvc $GOSH_PATH/task.tvc | tr -d ' ",' | sed -n '/code:/s/code://p')
PROFILE_CODE=$(/opt/gosh/contracts/tonos-cli -j decode stateinit --tvc $GOSH_PATH/../profile.tvc | tr -d ' ",' | sed -n '/code:/s/code://p')
PROFILEINDEX_CODE=$(/opt/gosh/contracts/tonos-cli -j decode stateinit --tvc $GOSH_PATH/../profileindex.tvc | tr -d ' ",' | sed -n '/code:/s/code://p')
PROFILEDAO_CODE=$(/opt/gosh/contracts/tonos-cli -j decode stateinit --tvc $GOSH_PATH/../profiledao.tvc | tr -d ' ",' | sed -n '/code:/s/code://p')
DAO_TAG_CODE=$(/opt/gosh/contracts/tonos-cli -j decode stateinit --tvc $GOSH_PATH/daotag.tvc | tr -d ' ",' | sed -n '/code:/s/code://p')
HELP_TAG_CODE=$(/opt/gosh/contracts/tonos-cli -j decode stateinit --tvc $GOSH_PATH/taggosh.tvc | tr -d ' ",' | sed -n '/code:/s/code://p')
TOPIC_CODE=$(/opt/gosh/contracts/tonos-cli -j decode stateinit --tvc $GOSH_PATH/topic.tvc | tr -d ' ",' | sed -n '/code:/s/code://p')
BIGTASK_CODE=$(/opt/gosh/contracts/tonos-cli -j decode stateinit --tvc $GOSH_PATH/bigtask.tvc | tr -d ' ",' | sed -n '/code:/s/code://p')
GRANT_CODE=$(/opt/gosh/contracts/tonos-cli -j decode stateinit --tvc $GOSH_PATH/grant.tvc | tr -d ' ",' | sed -n '/code:/s/code://p')
TAG_SUPPLY_CODE=$(/opt/gosh/contracts/tonos-cli -j decode stateinit --tvc $GOSH_PATH/tagsupply.tvc | tr -d ' ",' | sed -n '/code:/s/code://p')
...
VERSIONCONTROLLER_ADDR=$(/opt/gosh/contracts/tonos-cli -j genaddr --abi $VERSIONCONTROLLER_ABI --setkey "$seed" --save $VERSIONCONTROLLER_TVC | jq -M ".raw_address" | cut -d '"' -f 2)
...
/opt/gosh/contracts/tonos-cli callx --abi $GIVER_ABI --addr $GIVER_ADDR --keys "$GIVER_SEED" -m submitTransaction "{\"dest\": \"$VERSIONCONTROLLER_ADDR\", \"value\": 2000000000000, \"bounce\": false, \"allBalance\": false, \"payload\": \"\"}" || exit 1
...
/opt/gosh/contracts/tonos-cli deploy --abi $VERSIONCONTROLLER_ABI --sign "$seed" $VERSIONCONTROLLER_TVC "{}"
...
/opt/gosh/contracts/tonos-cli callx --abi $VERSIONCONTROLLER_ABI --addr $VERSIONCONTROLLER_ADDR --keys "$seed" -m setSystemContractCode "{\"code\": \"$SYSTEMCONTRACT_CODE\", \"version\": \"$GOSH_VERSION\"}" || exit 1
echo "     ====> Run setProfile"
...
/opt/gosh/contracts/tonos-cli callx --abi $VERSIONCONTROLLER_ABI --addr $VERSIONCONTROLLER_ADDR --keys "$seed" -m setProfile "{\"code\": \"$PROFILE_CODE\"}" || exit 1
...
/opt/gosh/contracts/tonos-cli callx --abi $VERSIONCONTROLLER_ABI --addr $VERSIONCONTROLLER_ADDR --keys "$seed" -m setProfileIndex "{\"code\": \"$PROFILEINDEX_CODE\"}" || exit 1
...
/opt/gosh/contracts/tonos-cli callx --abi $VERSIONCONTROLLER_ABI --addr $VERSIONCONTROLLER_ADDR --keys "$seed" -m setProfileDao "{\"code\": \"$PROFILEDAO_CODE\"}" || exit 1
...
/opt/gosh/contracts/tonos-cli callx --abi $VERSIONCONTROLLER_ABI --addr $VERSIONCONTROLLER_ADDR --keys "$seed" -m deploySystemContract "{\"version\": \"$GOSH_VERSION\"}" || exit 1
...
SYSTEMCONTRACT_ADDR=$(/opt/gosh/contracts/tonos-cli runx --addr $VERSIONCONTROLLER_ADDR --abi $VERSIONCONTROLLER_ABI  -m getSystemContractAddr "{\"version\": \"$GOSH_VERSION\"}" | sed -nr 's/.*"value0":[[:space:]]+"(.*)"/\1/p')
...
/opt/gosh/contracts/tonos-cli callx --abi $GIVER_ABI --addr $GIVER_ADDR --keys "$GIVER_SEED" -m submitTransaction "{\"dest\": \"$SYSTEMCONTRACT_ADDR\", \"value\": $GOSH_BALANCE, \"bounce\": false, \"allBalance\": false, \"payload\": \"\"}" || exit 1
...
/opt/gosh/contracts/tonos-cli callx --abi $SYSTEMCONTRACT_ABI --addr $SYSTEMCONTRACT_ADDR --keys "$seed" -m setFlag "{\"flag\":\"true\"}" || exit 1
...
/opt/gosh/contracts/tonos-cli callx --abi $SYSTEMCONTRACT_ABI --addr $SYSTEMCONTRACT_ADDR --keys "$seed" -m setTokenRoot "{\"code\":\"$TOKEN_ROOT_CODE\"}" || exit 1
...
/opt/gosh/contracts/tonos-cli callx --abi $SYSTEMCONTRACT_ABI --addr $SYSTEMCONTRACT_ADDR --keys "$seed" -m setTokenWallet "{\"code\":\"$TOKEN_WALLET_CODE\"}" || exit 1
...
/opt/gosh/contracts/tonos-cli callx --abi $SYSTEMCONTRACT_ABI --addr $SYSTEMCONTRACT_ADDR --keys "$seed" -m setTokenLocker "{\"code\":\"$TOKEN_LOCKER_CODE\"}" || exit 1
...
/opt/gosh/contracts/tonos-cli callx --abi $SYSTEMCONTRACT_ABI --addr $SYSTEMCONTRACT_ADDR --keys "$seed" -m setSMVPlatform "{\"code\":\"$LOCKER_PLATFORM_CODE\"}" || exit 1
...
/opt/gosh/contracts/tonos-cli callx --abi $SYSTEMCONTRACT_ABI --addr $SYSTEMCONTRACT_ADDR --keys "$seed" -m setSMVClient "{\"code\":\"$SMV_CLIENT_CODE\"}" || exit 1
...
/opt/gosh/contracts/tonos-cli callx --abi $SYSTEMCONTRACT_ABI --addr $SYSTEMCONTRACT_ADDR --keys "$seed" -m setSMVProposal "{\"code\":\"$SMV_PROPOSAL_CODE\"}" || exit 1
...
/opt/gosh/contracts/tonos-cli callx --abi $SYSTEMCONTRACT_ABI --addr $SYSTEMCONTRACT_ADDR --keys "$seed" -m setDiff "{\"code\":\"$DIFF_CODE\"}" || exit 1
...
/opt/gosh/contracts/tonos-cli callx --abi $SYSTEMCONTRACT_ABI --addr $SYSTEMCONTRACT_ADDR --keys "$seed" -m setRepository "{\"code\":\"$REPO_CODE\"}" || exit 1
...
/opt/gosh/contracts/tonos-cli callx --abi $SYSTEMCONTRACT_ABI --addr $SYSTEMCONTRACT_ADDR --keys "$seed" -m setCommit "{\"code\":\"$COMMIT_CODE\"}" || exit 1
...
/opt/gosh/contracts/tonos-cli callx --abi $SYSTEMCONTRACT_ABI --addr $SYSTEMCONTRACT_ADDR --keys "$seed" -m setSnapshot "{\"code\":\"$SNAPSHOT_CODE\"}" || exit 1
...
/opt/gosh/contracts/tonos-cli callx --abi $SYSTEMCONTRACT_ABI --addr $SYSTEMCONTRACT_ADDR --keys "$seed" -m setcontentSignature "{\"code\":\"$CONTENTSIG_CODE\"}" || exit 1
...
/opt/gosh/contracts/tonos-cli callx --abi $SYSTEMCONTRACT_ABI --addr $SYSTEMCONTRACT_ADDR --keys "$seed" -m setWallet "{\"code\":\"$WALLET_CODE\"}" || exit 1
...
/opt/gosh/contracts/tonos-cli callx --abi $SYSTEMCONTRACT_ABI --addr $SYSTEMCONTRACT_ADDR --keys "$seed" -m setDao "{\"code\":\"$DAO_CODE\"}" || exit 1
...
/opt/gosh/contracts/tonos-cli callx --abi $SYSTEMCONTRACT_ABI --addr $SYSTEMCONTRACT_ADDR --keys "$seed" -m setTree "{\"code\":\"$TREE_CODE\"}" || exit 1
...
/opt/gosh/contracts/tonos-cli callx --abi $SYSTEMCONTRACT_ABI --addr $SYSTEMCONTRACT_ADDR --keys "$seed" -m setTag "{\"code\":\"$TAG_CODE\"}" || exit 1
...
/opt/gosh/contracts/tonos-cli callx --abi $SYSTEMCONTRACT_ABI --addr $SYSTEMCONTRACT_ADDR --keys "$seed" -m setTask "{\"code\":\"$TASK_CODE\"}" || exit 1
...
/opt/gosh/contracts/tonos-cli callx --abi $SYSTEMCONTRACT_ABI --addr $SYSTEMCONTRACT_ADDR --keys "$seed" -m setDaoTag "{\"code\":\"$DAO_TAG_CODE\"}" || exit 1
...
/opt/gosh/contracts/tonos-cli callx --abi $SYSTEMCONTRACT_ABI --addr $SYSTEMCONTRACT_ADDR --keys "$seed" -m setHelpTag "{\"code\":\"$HELP_TAG_CODE\"}"  || exit 1
...
/opt/gosh/contracts/tonos-cli callx --abi $SYSTEMCONTRACT_ABI --addr $SYSTEMCONTRACT_ADDR --keys "$seed" -m setTopic "{\"code\":\"$TOPIC_CODE\"}"  || exit 1
...
/opt/gosh/contracts/tonos-cli callx --abi $SYSTEMCONTRACT_ABI --addr $SYSTEMCONTRACT_ADDR --keys "$seed" -m setBigTask "{\"code\":\"$BIGTASK_CODE\"}"  || exit 1
...
/opt/gosh/contracts/tonos-cli callx --abi $SYSTEMCONTRACT_ABI --addr $SYSTEMCONTRACT_ADDR --keys "$seed" -m setGrant "{\"code\":\"$GRANT_CODE\"}"  || exit 1
...
/opt/gosh/contracts/tonos-cli callx --abi $SYSTEMCONTRACT_ABI --addr $SYSTEMCONTRACT_ADDR --keys "$seed" -m setFlag "{\"flag\":\"false\"}"  || exit 1
...
/opt/gosh/contracts/tonos-cli callx --abi $GIVER_ABI --addr $GIVER_ADDR --keys "$GIVER_SEED" -m TheBigBang "{\"returnMoney\": \"$SYSTEMCONTRACT_ADDR\"}"  || exit 1