...
tonos-cli callx --abi $SYSTEMCONTRACT_ABI --addr $SYSTEMCONTRACT_ADDR --keys "$seed" -m setGrant "{\"code\":\"$GRANT_CODE\"}"  || exit 1 
...
tonos-cli callx --abi $SYSTEMCONTRACT_ABI --addr $SYSTEMCONTRACT_ADDR --keys "$seed" -m setTagSupplyTask "{\"code\":\"$TAG_SUPPLY_CODE\"}"  || exit 1 
