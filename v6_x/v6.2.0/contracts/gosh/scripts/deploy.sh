...
gosh-cli callx --abi $GIVER_ABI --addr $GIVER_ADDR --keys "$GIVER_SEED" -m submitTransaction "{\"dest\": \"$VERSIONCONTROLLER_ADDR\", \"value\": 2000000000000, \"bounce\": false, \"allBalance\": false, \"payload\": \"\"}" || exit 1 
...
gosh-cli deploy --abi $VERSIONCONTROLLER_ABI --sign "$seed" $VERSIONCONTROLLER_TVC "{}"
...
gosh-cli callx --abi $VERSIONCONTROLLER_ABI --addr $VERSIONCONTROLLER_ADDR --keys "$seed" -m setSystemContractCode "{\"code\": \"$SYSTEMCONTRACT_CODE\", \"version\": \"$GOSH_VERSION\"}" || exit 1 
echo "     ====> Run setProfile"
...
gosh-cli callx --abi $VERSIONCONTROLLER_ABI --addr $VERSIONCONTROLLER_ADDR --keys "$seed" -m setProfile "{\"code\": \"$PROFILE_CODE\"}" || exit 1 
...
gosh-cli callx --abi $VERSIONCONTROLLER_ABI --addr $VERSIONCONTROLLER_ADDR --keys "$seed" -m setProfileIndex "{\"code\": \"$PROFILEINDEX_CODE\"}" || exit 1 
...
gosh-cli callx --abi $VERSIONCONTROLLER_ABI --addr $VERSIONCONTROLLER_ADDR --keys "$seed" -m setProfileDao "{\"code\": \"$PROFILEDAO_CODE\"}" || exit 1 
...
gosh-cli callx --abi $VERSIONCONTROLLER_ABI --addr $VERSIONCONTROLLER_ADDR --keys "$seed" -m deploySystemContract "{\"version\": \"$GOSH_VERSION\"}" || exit 1 
...
SYSTEMCONTRACT_ADDR=$(gosh-cli runx --addr $VERSIONCONTROLLER_ADDR --abi $VERSIONCONTROLLER_ABI  -m getSystemContractAddr "{\"version\": \"$GOSH_VERSION\"}" | sed -nr 's/.*"value0":[[:space:]]+"(.*)"/\1/p')
...
gosh-cli callx --abi $GIVER_ABI --addr $GIVER_ADDR --keys "$GIVER_SEED" -m submitTransaction "{\"dest\": \"$SYSTEMCONTRACT_ADDR\", \"value\": $GOSH_BALANCE, \"bounce\": false, \"allBalance\": false, \"payload\": \"\"}" || exit 1 
...
gosh-cli callx --abi $SYSTEMCONTRACT_ABI --addr $SYSTEMCONTRACT_ADDR --keys "$seed" -m setFlag "{\"flag\":\"true\"}" || exit 1 
...
gosh-cli callx --abi $SYSTEMCONTRACT_ABI --addr $SYSTEMCONTRACT_ADDR --keys "$seed" -m setTokenRoot "{\"code\":\"$TOKEN_ROOT_CODE\"}" || exit 1 
...
gosh-cli callx --abi $SYSTEMCONTRACT_ABI --addr $SYSTEMCONTRACT_ADDR --keys "$seed" -m setTokenWallet "{\"code\":\"$TOKEN_WALLET_CODE\"}" || exit 1 
...
gosh-cli callx --abi $SYSTEMCONTRACT_ABI --addr $SYSTEMCONTRACT_ADDR --keys "$seed" -m setTokenLocker "{\"code\":\"$TOKEN_LOCKER_CODE\"}" || exit 1 
...
gosh-cli callx --abi $SYSTEMCONTRACT_ABI --addr $SYSTEMCONTRACT_ADDR --keys "$seed" -m setSMVPlatform "{\"code\":\"$LOCKER_PLATFORM_CODE\"}" || exit 1 
...
gosh-cli callx --abi $SYSTEMCONTRACT_ABI --addr $SYSTEMCONTRACT_ADDR --keys "$seed" -m setSMVClient "{\"code\":\"$SMV_CLIENT_CODE\"}" || exit 1 
...
gosh-cli callx --abi $SYSTEMCONTRACT_ABI --addr $SYSTEMCONTRACT_ADDR --keys "$seed" -m setSMVProposal "{\"code\":\"$SMV_PROPOSAL_CODE\"}" || exit 1 
...
gosh-cli callx --abi $SYSTEMCONTRACT_ABI --addr $SYSTEMCONTRACT_ADDR --keys "$seed" -m setDiff "{\"code\":\"$DIFF_CODE\"}" || exit 1 
...
gosh-cli callx --abi $SYSTEMCONTRACT_ABI --addr $SYSTEMCONTRACT_ADDR --keys "$seed" -m setRepository "{\"code\":\"$REPO_CODE\"}" || exit 1 
...
gosh-cli callx --abi $SYSTEMCONTRACT_ABI --addr $SYSTEMCONTRACT_ADDR --keys "$seed" -m setCommit "{\"code\":\"$COMMIT_CODE\"}" || exit 1 
...
gosh-cli callx --abi $SYSTEMCONTRACT_ABI --addr $SYSTEMCONTRACT_ADDR --keys "$seed" -m setSnapshot "{\"code\":\"$SNAPSHOT_CODE\"}" || exit 1 
...
gosh-cli callx --abi $SYSTEMCONTRACT_ABI --addr $SYSTEMCONTRACT_ADDR --keys "$seed" -m setcontentSignature "{\"code\":\"$CONTENTSIG_CODE\"}" || exit 1 
...
gosh-cli callx --abi $SYSTEMCONTRACT_ABI --addr $SYSTEMCONTRACT_ADDR --keys "$seed" -m setWallet "{\"code\":\"$WALLET_CODE\"}" || exit 1 
...
gosh-cli callx --abi $SYSTEMCONTRACT_ABI --addr $SYSTEMCONTRACT_ADDR --keys "$seed" -m setDao "{\"code\":\"$DAO_CODE\"}" || exit 1 
...
gosh-cli callx --abi $SYSTEMCONTRACT_ABI --addr $SYSTEMCONTRACT_ADDR --keys "$seed" -m setTree "{\"code\":\"$TREE_CODE\"}" || exit 1 
...
gosh-cli callx --abi $SYSTEMCONTRACT_ABI --addr $SYSTEMCONTRACT_ADDR --keys "$seed" -m setTag "{\"code\":\"$TAG_CODE\"}" || exit 1 
...
gosh-cli callx --abi $SYSTEMCONTRACT_ABI --addr $SYSTEMCONTRACT_ADDR --keys "$seed" -m setTask "{\"code\":\"$TASK_CODE\"}" || exit 1 
...
gosh-cli callx --abi $SYSTEMCONTRACT_ABI --addr $SYSTEMCONTRACT_ADDR --keys "$seed" -m setDaoTag "{\"code\":\"$DAO_TAG_CODE\"}" || exit 1 
...
gosh-cli callx --abi $SYSTEMCONTRACT_ABI --addr $SYSTEMCONTRACT_ADDR --keys "$seed" -m setHelpTag "{\"code\":\"$HELP_TAG_CODE\"}"  || exit 1 
...
gosh-cli callx --abi $SYSTEMCONTRACT_ABI --addr $SYSTEMCONTRACT_ADDR --keys "$seed" -m setTopic "{\"code\":\"$TOPIC_CODE\"}"  || exit 1 
...
gosh-cli callx --abi $SYSTEMCONTRACT_ABI --addr $SYSTEMCONTRACT_ADDR --keys "$seed" -m setFlag "{\"flag\":\"false\"}"  || exit 1 
...
gosh-cli callx --abi $GIVER_ABI --addr $GIVER_ADDR --keys "$GIVER_SEED" -m TheBigBang "{\"returnMoney\": \"$SYSTEMCONTRACT_ADDR\"}"  || exit 1 