...
*/
    ...
    function getCellForDestroyTopic(
        address topic,
        string comment,
        optional(uint32) time
    ) external pure returns(TvmCell) {
        uint256 proposalKind = DESTROY_TOPIC_KIND;
        if (time.hasValue() == false) { time = block.timestamp; }
        return abi.encode(proposalKind, topic, comment, time.get());
    }

    function _destroyTopic (address topic) private view  {
        require(_tombstone == false, ERR_TOMBSTONE);
        require(_limited == false, ERR_WALLET_LIMITED);
...
//            }  else
            ...
            if (kind == DESTROY_TOPIC_KIND) {
                (,address topic,,) = abi.decode(propData,(uint256, address, string, uint32));
                _destroyTopic(topic);
            }
        }
    }
