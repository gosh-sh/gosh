...
library GoshLib {
    ...
    function composeRepoWalletInitData(TvmCell code, string repo, address root, address walletowner) private returns (TvmCell) {
        ...
        b.store(repo);
        TvmCell data = tvm.setCodeSalt(code, b.toCell());
...
//Address part
    ...
    function calculateTagSupplyAddress(TvmCell code, address dao, uint256 namehash) public returns(address) {
        TvmCell s1 = composeTagSupplyStateInit(code, dao, namehash);
        return address.makeAddrStd(0, tvm.hash(s1));
    }
    ...
    function calculateCCWalletAddress(TvmCell code, address versionController, uint256 pubkey) public returns(address) {
        TvmCell s1 = composeCCWalletStateInit(code, versionController, pubkey);
        return address.makeAddrStd(0, tvm.hash(s1));
    }

    function composeCCWalletStateInit(TvmCell code, address versionController, uint256 pubkey) public returns(TvmCell) {
        return tvm.buildStateInit({
