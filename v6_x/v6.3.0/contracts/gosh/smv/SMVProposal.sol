...
function getDestroyTopicProposalParams () external view
         returns(uint proposalKind, address topic)
{
     (proposalKind, topic,,) = abi.decode(propData, (uint256, address, string, uint32));
}

function getDestroyTopicProposalParamsData (TvmCell Data) external pure
         returns(uint proposalKind, address topic)
{
     (proposalKind, topic,,) = abi.decode(Data, (uint256, address, string, uint32));
}
