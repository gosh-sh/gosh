...
import { SunEditor } from "../../components/Editors/SunEditor";
import { JoditEditor } from "../../components/Editors/JoditEditor";
...
const BlobCommitForm = (props: TBlobCommitFormProps) => {
  ...
  const [initialFormikValues, setInitialFormikValues] =
    useState<TBlobCommitFormValues>();
  ...
  const getInitialValues = async () => {
    ...
    switch (filetype) {
      case Filetype.MARKDOWN:
        if (!isHTML(initialValues.content))
          initialValues.content = await markdown2html(initialValues.content);
        break;
  ...
  const getValidationSchema = () => {
    ...
    const version_3_0_0 = {
      ...
      assigners: yup
        .array()
        .test("check-task", "Min 1 assigner", function (value) {
  ...
  const onFilenameBlur = (
    ...
    if (!values.title) {
      setFieldValue(
        "title",
        `Create ${e.target.value}${filetype ? `.${filetype}` : ""}`,
      );
    }
  };
              ...
              <div className="flex flex-wrap gap-3 items-baseline justify-between ">
                <div className="flex flex-wrap items-baseline gap-y-2">
                  ...
                  <div className={`relative !max-w-[400px]`}>
                    ...
                    {values.name &&
                      filetype &&
                      !values.name.endsWith(filetype) && (
                        <div className="flex mt-[-2.375rem] pointer-events-none top-0 right-0 w-full overflow-hidden leading-9 border border-transparent text-black/30 px-4 text-sm mx-[1px]">
                          <div className="grow-1 shrink-0 text-transparent max-w-[calc(100%-1.75rem)]">
                            {values.name}
                          </div>
                          <div
                            className={`grow-0 shrink-0 ${filetype && "w-[1.75rem]"}`}
                          >
                            {filetype && `.${filetype}`}
                          </div>
                        </div>
