...
import { BaseField, FormikInput } from "../Formik";
...
import { UserSelect } from "../../components/UserSelect/UserSelect_";

type TFormParticipant = {
  dao_name: string;
  dao_version: string;
  repo_names: string[];
};
...
const getRepositoryOptions = async (params: {
  ...
  const query = await sc.getRepository({
    path: `${application.dao_name}/${input}`,
  });
...
const HackathonApplicationSubmit = (props: TApplicationSubmitProps) => {
  ...
  const getFormValidationSchema = () => {
    const form_data =
      application_form?.fields.map((field) => [
        field.name,
        yup.string().required(),
      ]) || [];
  ...
  const getFormInitialValues = () => {
    const form_data =
      application_form?.fields.map((field) => [field.name, ""]) || [];
    return {
      applications: [],
                ...
                <div>
                  ...
                  >
                    <AsyncSelect
                      ...
                      isDisabled={
                        form.isSubmitting ||
                        !values.applications[index].dao_name
                      }
                      cacheOptions={false}
