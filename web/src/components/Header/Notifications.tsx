...
const Notifications = () => {
  ...
  const { data, setFilters, updateUserNotification } =
  ...
  return (
    <AnimatePresence mode="wait">
      <Popover as="div" className="md:relative">
        ...
        <PopoverBackdrop
          transition
          className="fixed transition-all inset-0 bg-black/15 z-[1000] transition duration-100 ease-out data-[closed]:scale-100 data-[closed]:opacity-0"
        />
        <PopoverPanel
          as={motion.div}
          className="fixed left-[88px] bottom-4 mt-2 z-[1010] w-screen max-w-lg px-5 md:px-0"
          initial={{ opacity: 0, translateY: "0.25rem" }}
          animate={{ opacity: 1, translateY: 0 }}
          exit={{ opacity: 0, translateY: "0.25rem" }}
          transition
        >
          <div className="rounded-xl shadow-sm shadow-[#7c8db5]/5 bg-white p-4">
                ...
                >
                  ...
                  {data.daolist.map((item, index) => (
                    <Menu.Item
                      ...
                      as="div"
                      className="text-sm p-2 hover:bg-gray-fafafd cursor-pointer rounded-lg"
                      onClick={() => onFilterByDaoClick(item.daoname)}
                    >
                      {item.selected && (
                        <FontAwesomeIcon
                          icon={faCheck}
