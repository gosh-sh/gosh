...
const RequestDaoMembershipModal = () => {
  ...
  const disabled =
    !dao.details.isAskMembershipOn || !member.isReady || member.isMember;
  const is_fetching =
    dao.isFetching ||
    !dao.details.isReady ||
    (!!user.profile && !member.isReady);
  ...
  return (
    <Dialog.Panel className="relative rounded-xl bg-white p-10 w-full max-w-md">
      ...
      {is_fetching && (
        <Loader className="text-center">Fetching data, please wait</Loader>
      )}
      ...
      {!is_fetching && !dao.details.isAskMembershipOn && (
        <Alert variant="danger" className="mb-4">
          ...
          <p className="mt-1 text-xs">
            DAO has disabled request membership, you should contact one of the
            DAO members to ask to invite you
          </p>
        </Alert>
      )}
