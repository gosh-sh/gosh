...
export function useDaoMember(
  ...
  const setStatus2 = useSetRecoilState(
    appToastStatusSelector("__waitdaoready"),
  );
    ...
    try {
      ...
      const profiles = await executeByChunk(
        ...
        async (item) => {
          ...
          const member = dao.members?.find(
            (v) => v.profile.address === address,
          );
          if (!member) {
            throw new GoshError("Profile error", {
...
export function useUpgradeDaoComplete() {
  ...
  const upgrade = useCallback(async () => {
    try {
      ...
      if (daoaddrPrev) {
        const _prev = await getSystemContract().getDao({
          address: daoaddrPrev,
        });
    ...
    async (params: {
      ...
      try {
        ...
        for (const tag_param of params.tags) {
          const found = tags_current.find(
            ({ name }) => name === tag_param.name,
          );
...
export function useMintDaoTokens() {
  ...
  const mint = useCallback(
    async (params: { amount: number; comment?: string }) => {
      const { amount } = params;
      const comment =
        params.comment || `Mint ${amount.toLocaleString()} tokens`;
  ...
  const send = useCallback(
    async (params: {
      ...
      try {
        ...
        const comment =
          params.comment ||
          `Send ${amount} ${_txtTokens} tokens to ${username}`;
        const kwargs = {
