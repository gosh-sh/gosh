...
export function useCreateHackathon() {
  ...
  const create = useCallback(
    async (params: {
      ...
      const { name, type, description, prize, dates, expert_tags, comment } =
        params;
  ...
  const getBlockchainItems = async (params: {
    ...
    const items = await executeByChunk<{ id: string }, THackathonDetails>(
      ...
      results.filter(({ id }) => {
        return (
          id !==
          "0:0a23509cd7a69fffb86949a0c90a7686c1d24df26ffc290154e56cbee6d5fab2"
        );
      }),
      MAX_PARALLEL_READ,
      async ({ id }) => {
...
export function useHackathon(
  ...
  const { details: dao } = useDao();
  const [hackathons, setHakathons] = useRecoilState(
    daoHackathonListSelector(dao.name),
  );
            ...
            apps_submitted: {
              items: apps_submitted.map((app) => {
                const exists = item.apps_submitted.items.find((v) => {
                  return (
                    v.dao_name === app.dao_name && v.repo_name === app.repo_name
                  );
                });
                if (exists) {
                  return { ...exists, ...app };
...
export function useApplicationForm() {
  const encrypt = async (params: { owners: string[]; data: Object }) => {
    ...
    const encrypted_data_encryption_key = owners.map((pubkey) => {
      ...
      const pubkey_curve25519 =
        sodium.crypto_sign_ed25519_pk_to_curve25519(pubkey_uint8);
  ...
  const decrypt = async (params: {
    ...
    const encrypted_data_encryption_key =
      application_form.encrypted_key[user_pubkey];
    ...
    const ed25519_skpk = sodium.from_hex(
      `${user_keypair.secret}${user_keypair.public}`,
    );
    const user_secret =
      sodium.crypto_sign_ed25519_sk_to_curve25519(ed25519_skpk);
...
export function useHackathonVoting() {
  ...
  const checked_apps =
    hackathon?.apps_submitted.items.filter(
            ...
            member_voting_state: {
              ...
              karma_added: user_karma_added.map((app: any) => {
                const found = curr?.karma_added.find((f) => {
                  return (
                    f.dao_name === app.dao_name && f.repo_name === app.repo_name
                  );
                });
                if (found) {
  ...
  const approveApps = async () => {
    try {
      if (checked_apps.length === 0) {
        throw new GoshError(
          "Value error",
          "No applications selected for voting",
        );
      }
      if (!hackathon) {
