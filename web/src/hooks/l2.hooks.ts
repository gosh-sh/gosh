...
export function useL2Transfer(options: { initialize?: boolean } = {}) {
  ...
  const [status, setStatus] = useRecoilState(
    appToastStatusSelector("__l2transfer"),
  );
  ...
  const getErc20Approvement = async (rootaddr: string, walletaddr: string) => {
    ...
    const result: any = await elock.methods
      ...
      .call();
    return {
      commission: result.commission as bigint,
  ...
  const depositEth = async () => {
    try {
      ...
      await data.web3.instance.eth.sendTransaction({
        from: data.web3.address,
        to: AppConfig.elockaddr,
        value: data.web3.instance.utils.toWei(
          data.summary.from.amount,
          "ether",
        ),
        data: edata,
  ...
  const receiveGosh = async () => {
    try {
      ...
      const token_root = AppConfig.getTIP3RootFlex(
        data.summary.to.token.rootaddr,
      );
      const wallet = await token_root.getWallet({
      ...
      if (!wait_minted) {
        throw new GoshError(
          "Timeout error",
          "Tokens did not arive during timeout",
        );
      }
  ...
  const onSetCommissionsCallback = useCallback(() => {
    ...
    if (
      summary.to.token.network !== EL2Network.GOSH &&
      summary.to.token.rootaddr
    ) {
      commission = 0n;
    }
  ...
  useEffect(() => {
    ...
    return () => {
      if (initialize) {
        ...
        data.web3.instance?.provider?.removeListener(
          "accountsChanged",
          () => {},
        );
