...
export function useCreateRepository() {
  ...
  const setRepositories = useSetRecoilState(
    daoRepositoryListSelector(dao.name),
  );
  ...
  const create = useCallback(
    async (name: string, description?: string, expert_tags?: string[]) => {
      try {
        ...
        // update state
        if (alone) {
          const wait = await whileFinite(
            async () => await account.isDeployed(),
          );
          if (!wait) {
            throw new GoshError(
              "Timeout error",
...
export function useUpdateBranchConfig() {
  ...
  const removeProtection = useCallback(
    async (params: { repo_name: string; branch: string; comment?: string }) => {
