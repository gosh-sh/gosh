...
import { useBranches, usePush } from "react-gosh";
  ...
  const onPush = async (values: TBlobCommitFormValues) => {
    try {
      ...
      const blobObject = {
        treepath: [
          "",
          `${treepath ? `${treepath}/` : ""}${name.match(new RegExp(`\.${filetype}$`)) ? name : name.concat(".", filetype)}`,
        ],
        original: "",
        modified: content,
      };
    ...
    <div>
      <BlobCommitForm
        ...
        initialValues={{
          ...
          title: "",
          content: "",
          ...(isPullRequest !== undefined ? { isPullRequest } : {}),
        }}
        urlBack={urlBack}
