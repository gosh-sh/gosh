...
const TaskCreatePage = () => {
    ...
    const [payments, setPayments] = useState<{ parts: number; struct: TStruct }>({
        parts: 1,
        struct: { assign: [], review: [], manager: [] },
    })
    ...
    const updateStructState = useCallback(
        debounce((values: TFormValues) => {
            ...
            setPayments({
                struct: _struct,
                parts: values.vestingDuration > 0 ? values.vestingParts : 1,
            })
        }, 500),
        [],
    )
                    ...
                    <Form>
                        ...
                        <div className="mt-14">
                            <h3 className="font-medium">
                                Default task tokens distribution
                            </h3>
                            <div className="text-sm text-gray-7c8db5">
                                If a pull request request is attached to a task, tokens
            ...
            <div className="mt-14">
                ...
                <Chart
                    options={{
                        ...
                        xaxis: {
                            categories: Array.from(new Array(payments.parts)).map(
                                (_, i) => i + 1,
                            ),
                        },
                    }}
                    series={[
                        {
                            name: 'assign',
                            data: payments.struct.assign.map(({ grant }) => grant),
                        },
                        {
                            name: 'review',
                            data: payments.struct.review.map(({ grant }) => grant),
                        },
                        {
                            name: 'manager',
                            data: payments.struct.manager.map(({ grant }) => grant),
                        },
                    ]}
                    height={190}
                />
