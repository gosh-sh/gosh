...
const DaoEventVotingForm = (props: TDaoEventVotingFormProps) => {
  ...
  const getMaxAmount = useCallback(() => {
    ...
    return Math.min(
      balance.total + balance.regular - event.votes.yours,
        ...
        <Form>
          ...
          <div className="mt-4">
            <Button
              className="w-full"
              type="submit"
              disabled={isSubmitting || values.approve === undefined || start > 0}
              isLoading={isSubmitting}
            >
              {start > 0 ? `${start.toLocaleString()}s before voting` : 'Send vote'}
            </Button>
          </div>
        </Form>
      )}
