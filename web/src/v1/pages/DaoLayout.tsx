...
import { ToastOptionsShortcuts, getIdenticonAvatar } from '../../helpers'
...
import { ToastOptions, UpdateOptions, toast, useToast } from 'react-toastify'
import { ToastError } from '../../components/Toast'

const DaoLayout = () => {
    const { daoName } = useParams()
    const toastId = useRef<string | number>('')
    const { message } = useDao()
    ...
    useEffect(() => {
        toastId.current = toast('', { autoClose: 1 })
    }, [])
        ...
        if (message) {
            ...
            switch (type) {
                case 'error':
                    ...
                    break
                case 'pending':
                    toast.update(toastId.current, { isLoading: true, render: data })
                    break
            }
        }
    }, [message, toastId.current])
...
export default withRouteAnimation(withPin(DaoLayout, { redirect: false }))
