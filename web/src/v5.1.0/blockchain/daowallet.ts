...
export class DaoWallet extends BaseContract {
  ...
  async getBalance() {
    ...
    const { m_pseudoDAOVoteBalance } = await this.runLocal(
      'm_pseudoDAOVoteBalance',
      {},
    )
  ...
  async upgradeTask(params: {
    ...
    if (cell) {
      ...
      await this.run('startProposalForTaskUpgrade', {
        ...cellParams,
        reviewers,
        num_clients: await this.smvClientsCount(),
      })
  ...
  async getSnapshot(params: {
    address?: string
    data?: { branch: string; repo_addr: string; filename: string }
  }) {
    ...
    if (!_address) {
      const { branch, repo_addr, filename } = data!
      const { value0 } = await this.runLocal(
        'getSnapshotAddr',
        { branch, repo: repo_addr, name: filename },
        undefined,
        { useCachedBoc: true },
      )
      _address = value0
  ...
  async createSnapshot(params: {
    ...
    const { branch, commit_name, repo_addr, filename, content, ipfs_url } =
      params
