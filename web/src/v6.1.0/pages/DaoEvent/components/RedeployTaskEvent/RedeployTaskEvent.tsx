...
type TRedeployTaskEventProps = {
  data: any
}

const RedeployTaskEvent = (props: TRedeployTaskEventProps) => {
  const { data } = props
  ...
  return (
    <div className="flex flex-col gap-2 py-3">
      <div className="flex items-center gap-6">
        ...
        <div className="text-sm">{data.nametask}</div>
      </div>
      <div className="flex items-center gap-6">
      ...
      <div className="flex items-center gap-6">
        ...
        <div className="text-sm">{data.tags.map((v: string) => `#${v}`).join(', ')}</div>
      </div>
      <div className="flex items-center gap-6">
        <div className="basis-5/12 xl:basis-2/12 text-xs text-gray-53596d">Reward</div>
