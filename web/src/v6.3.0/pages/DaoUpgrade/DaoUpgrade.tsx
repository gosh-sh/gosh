    ...
    <div>
      ...
      {!dao.details.isUpgraded && (
        <Alert variant="danger" className="mb-4">
          You should complete current DAO upgrade process to upgrade to another version
        </Alert>
      )}
      ...
      <Formik
        initialValues={{
          version: versions
            ? versions.filter((v) => DISABLED_VERSIONS.indexOf(v) < 0).slice(-1)[0]
            : '',
          comment: '',
        }}
          ...
          <Form>
            ...
            <div>
              <Field
                ...
                disabled={isSubmitting || !versions?.length || !dao.details.isUpgraded}
              >
                {versions?.map((version, index) => (
                  ...
                  >
                    {version}
                    {DISABLED_VERSIONS.indexOf(version) >= 0 && ' (Not available)'}
                  </option>
                ))}
              </Field>
            </div>
